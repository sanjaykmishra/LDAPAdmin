...
CREATE TABLE access_review_campaigns (
    ...
    CONSTRAINT chk_arc_status   CHECK (status IN ('DRAFT', 'ACTIVE', 'CLOSED', 'CANCELLED'))
);
...
| Table | Purpose |
|-------|---------|
| `access_review_campaigns` | Top-level campaign definition with deadline, status, and ownership |
...
| `access_review_decisions` | One row per member per group. Pre-populated when the campaign is activated by snapshotting current members from LDAP. `decision` is NULL until the reviewer acts. |
...
Responsibilities:
1. **Create campaign** (DRAFT) — validate name, deadline > now, groups exist in LDAP
...
3. **Close campaign** — transition ACTIVE → CLOSED:
   ...
   - Set `completedAt`, record `CAMPAIGN_CLOSED` audit event
4. **Cancel campaign** — transition DRAFT/ACTIVE → CANCELLED
...
- `notifyDeadlineApproaching(campaign)` — for scheduled reminder (future enhancement)
- `notifyCampaignClosed(campaign)` — summary to campaign creator
...
| DTO | Purpose |
|-----|---------|
| `CreateCampaignRequest` | name, description, deadline, autoRevoke, groups (list of `{groupDn, memberAttribute, reviewerAccountId}`) |
| `CampaignSummaryDto` | id, name, status, deadline, createdAt, progress (total/confirmed/revoked/pending counts) |
| `CampaignDetailDto` | Full campaign + list of `ReviewGroupDto` |
| `ReviewGroupDto` | id, groupDn, groupName, reviewerUsername, decisionCounts |
| `DecisionDto` | id, memberDn, memberDisplay, decision, comment, decidedAt, revokedAt |
...
| `GET` | `/{campaignId}/export` | Export decisions as CSV | `access_review.manage` |
...
The export endpoint generates a CSV with columns:

```
Campaign, Group DN, Group Name, Member DN, Member Display, Decision, Comment, Decided At, Revoked At
```

This reuses the existing CSV generation pattern from `BulkUserService` / `ReportExecutionService`. The export serves as compliance evidence.
...
export const exportCampaign = (dirId, campaignId) =>
  client.get(`${base(dirId)}/${campaignId}/export`, { responseType: 'blob' })
```
...
- Form: name, description, deadline picker, auto-revoke toggle
...
- Campaign header: name, status badge, deadline (with countdown), description
...
- Action buttons: Activate (if DRAFT), Close (if ACTIVE), Cancel, Export CSV
...
1. **Database migration** (`V20`) — tables + feature key constraint update
2. **Enums** — `CampaignStatus`, `ReviewDecision`, add to `FeatureKey` and `AuditAction`
3. **Entities** — `AccessReviewCampaign`, `AccessReviewGroup`, `AccessReviewDecision`
4. **Repositories** — three JPA repositories
5. **DTOs** — request/response records
6. **Services** — `AccessReviewCampaignService`, `AccessReviewDecisionService`, `AccessReviewNotificationService`
7. **Controller** — `AccessReviewController`
8. **Service tests** — unit tests for campaign lifecycle (create → activate → decide → close)
9. **Controller tests** — MockMvc tests following `BaseControllerTest` pattern
10. **Frontend API module** — `accessReviews.js`
11. **Frontend views** — list → create → detail → decisions
12. **Router + navigation** — wire up routes and sidebar link
...
| Entities | `entity/AccessReviewCampaign.java`, `entity/AccessReviewGroup.java`, `entity/AccessReviewDecision.java` |
| Repos | `repository/AccessReviewCampaignRepository.java`, `AccessReviewGroupRepository.java`, `AccessReviewDecisionRepository.java` |
| DTOs | `dto/accessreview/CreateCampaignRequest.java`, `CampaignSummaryDto.java`, `CampaignDetailDto.java`, `ReviewGroupDto.java`, `DecisionDto.java`, `SubmitDecisionRequest.java`, `BulkDecisionRequest.java`, `CampaignProgressDto.java` |
| Services | `service/AccessReviewCampaignService.java`, `service/AccessReviewDecisionService.java`, `service/AccessReviewNotificationService.java` |
| Controller | `controller/directory/AccessReviewController.java` |
| Tests | `service/AccessReviewCampaignServiceTest.java`, `service/AccessReviewDecisionServiceTest.java`, `controller/AccessReviewControllerTest.java` |
...
| Auto-revoke removes critical memberships | Default `auto_revoke = false`; activation confirmation dialog warns about auto-revoke; audit trail records every removal |
| Campaign deadline gaming | Deadline is server-enforced; decisions after deadline are rejected (status must be ACTIVE) |
...
- **Scheduled deadline reminders** — cron job sends email N days before deadline (reuse `ScheduledReportJob` infrastructure)
