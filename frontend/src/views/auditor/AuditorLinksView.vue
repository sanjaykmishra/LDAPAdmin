  ...
  <div class="p-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Manage read-only evidence sharing links for auditors</p>
      </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-200 bg-gray-50">
              ...
              <th class="text-left py-2 px-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">URL</th>
            ...
            <tr v-for="link in links" :key="link.id" class="border-b border-gray-100 hover:bg-gray-50/50">
              ...
              <td class="py-2 px-4">
                <div class="flex items-center gap-1.5">
                  <span class="text-[10px] text-gray-500 font-mono truncate max-w-[200px]" :title="linkUrl(link)">{{ linkUrl(link) }}</span>
                  <button @click="copyToClipboard(linkUrl(link))" class="text-gray-400 hover:text-blue-600 shrink-0" title="Copy URL">
...
function linkUrl(link) {
  return `${window.location.origin}/auditor/${link.token}`
}
